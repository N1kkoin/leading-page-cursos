...
Papa.parse(url, {
  ...
  complete: function (results) {
    ...
    data.filter(row => row['Cursos'] === 'Pós Marketing').forEach(row => {
      ...
      if (row['Nome PosMarketing'] && row['Foto PosMarketing'] && row['Cargo PosMarketing']) {
        ...
        cardDiv.innerHTML = `
                  ...
                  <div class="teacherCurriculum">
                      <a class="teacherCurriculumLinks" href="${row['Contato PosMarketing']}" target="_blank" title="Contato"><i class="fa-solid fa-comment"></i></a>
                      <a class="teacherCurriculumLinks" href="${row['Currículo PosMarketing']}" target="_blank" title="Currículo"><i class="ai ai-lattes"></i>Currículo</a>
                  </div>
              `;
