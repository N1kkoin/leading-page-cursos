...
Papa.parse(url, {
  ...
  complete: function (results) {
    ...
    data.filter(row => row['Cursos'] === 'Pós EstraFiscal').forEach(row => {
...
Papa.parse(url3, {
  ...
  complete: function (results) {
    ...
    data.forEach(row => {
      if (row['Nome EstraFiscal'] && row['Foto EstraFiscal'] && row['Cargo EstraFiscal']) {
        ...
        cardDiv.innerHTML = `
                  <img class="teacherImg" src="${row['Foto EstraFiscal']}" alt="${row['Nome']}">
                  <h3>${row['Nome EstraFiscal']}</h3>
                  <p>${row['Cargo EstraFiscal']}</p>
                  <div class="teacherCurriculum">
                      <a class="teacherCurriculumLinks" href="${row['Contato EstraFiscal']}" target="_blank" title="Contato"><i class="fa-solid fa-comment"></i></a>
                      <a class="teacherCurriculumLinks" href="${row['Currículo EstraFiscal']}" target="_blank" title="Currículo"><i class="ai ai-lattes"></i>Currículo</a>
                  </div>
              `;
